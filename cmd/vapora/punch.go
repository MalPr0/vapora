...
func runPunch(args []string) error {
	...
	// A session writes to this stream only when nobody is drawing a screen.
	// Deciding here, rather than after the front end is chosen, is what keeps
	// anything the transport says off a painted terminal.
	var plainOutput io.Writer
	...
	open := &channel{
		conn:      conn,
		mux:       mux,
		session:   punch.NewSession(mux, codec, plainOutput),
